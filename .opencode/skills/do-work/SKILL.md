...
```bash
xcodebuild -project "SubTimer.xcodeproj" -scheme SubTimer -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -quiet build 2>&1
  
xcodebuild test -scheme SubTimer -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:SubTimerTests 2>&1
```
