...
struct TimerView: View {
    ...
    private var preferredTimeDisplay: some View {
        VStack(spacing: 8) {
            ...
            if let longestPlayingPlayer = activePlayers.max(by: { $0.currentPlayDuration < $1.currentPlayDuration }) {
                let timeRemaining = TimeInterval(configuration.preferredPlayTimeSeconds) - longestPlayingPlayer.currentPlayDuration
                ...
                if configuration.preferredPlayTimeSeconds > 0 {
                    HStack(spacing: 4) {
                        ...
                        Text(isOverTime ? "Over by \(formatTime(abs(timeRemaining)))" : "Preferred: \(formatTime(TimeInterval(configuration.preferredPlayTimeSeconds)))")
                    }
                    .font(.subheadline)
    ...
    private func activePlayerRow(player: Player) -> some View {
        let isNextToSubOut = activePlayers.max(by: { $0.currentPlayDuration < $1.currentPlayDuration })?.id == player.id
        ...
        return Button {
            ...
            .padding()
            .background(isNextToSubOut ? Color.orange.opacity(0.1) : Color(uiColor: .secondarySystemBackground))
            .cornerRadius(8)
        }
        .buttonStyle(.plain)
    }
    ...
    private func performAutomaticSubstitution() {
        guard let playerToSubOut = activePlayers.max(by: { $0.currentPlayDuration < $1.currentPlayDuration }),
              let playerToSubIn = benchedPlayers.first else {
            return
        }
